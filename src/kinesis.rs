...
pub fn get_client(aws_profile: &str, region: &str) -> Option<KinesisClient> {

    match Region::from_str(region) {
        Ok(aws_region) => {
            if aws_profile.is_empty() {
                ChainProvider::new();
                Some(KinesisClient::new(aws_region ))
            } else {
                ...
                match _profile_provider {
                    Ok(mut _prov) => {
                        ...
                        Some(KinesisClient::new_with(HttpClient::new().expect("failed to create request dispatcher"), _prov, aws_region))
                    },
                    Err(_e) => {
