...
pub fn get_client(aws_profile: &str, region: &str) -> Option<RdsClient> {

    match Region::from_str(region) {
        Ok(aws_region) => {
            if aws_profile.is_empty() {
                ChainProvider::new();
                Some(RdsClient::new(aws_region))
            } else {
                ...
                match _profile_provider {
                    Ok(mut _prov) => {
                        ...
                        Some(RdsClient::new_with(HttpClient::new().expect("failed to create request dispatcher"), _prov, aws_region))
                    },
                    Err(_e) => {
