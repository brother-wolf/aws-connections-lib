...
pub fn get_client(aws_profile: &str, region: &str) -> Result<CloudWatchClient, ClientError> {

    match Region::from_str(region) {
        Ok(aws_region) => {
            if aws_profile.is_empty() {
                ChainProvider::new();
                Ok(CloudWatchClient::new(aws_region ))
            } else {
                ...
                match _profile_provider {
                    Ok(mut _prov) => {
                        ...
                        Ok(CloudWatchClient::new_with(HttpClient::new().expect("failed to create request dispatcher"), _prov, aws_region))
                    },
