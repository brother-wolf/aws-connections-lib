...
pub fn get_client(aws_profile: &str, region: &str) -> Result<SqsClient, ClientError> {

    match Region::from_str(region) {
        Ok(aws_region) => {
            if aws_profile.is_empty() {
                ChainProvider::new();
                Ok(SqsClient::new(aws_region ))
            } else {
                ...
                match _profile_provider {
                    Ok(mut _prov) => {
                        ...
                        Ok(SqsClient::new_with(HttpClient::new().expect("failed to create request dispatcher"), _prov, aws_region))
                    },
